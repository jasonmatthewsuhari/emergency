...
export async function extractPageSignals(url: string): Promise<PageSignals> {
  const res = await fetch(url, {
    headers: {
      'User-Agent': 'Mozilla/5.0 (compatible; SightlineBot/1.0)',
      Accept: 'text/html',
    },
...
export async function buildCompanyBrief(url: string): Promise<CompanyBrief> {
  ...
  const res = await fetch(ANTHROPIC_API_URL, {
    ...
    body: JSON.stringify({
      model: process.env.ANTHROPIC_BRIEF_MODEL || DEFAULT_MODEL,
      max_tokens: 1024,
      system: BRIEF_SYSTEM,
  ...
  try {
    ...
    if (fence) {
      ...
      throw new Error(`Could not parse Claude response as JSON: ${raw.slice(0, 400)}`)
    }
  }
