import type { Building, BuildingPoiCategory, LatLng, RoadKind, RoadSegment, StreetFixture, StreetFixtureKind, TrafficPoint, VegetationFeature, VegetationKind } from '@/types'
...
export function buildRoadQuery(center: LatLng, radiusKm: number): string {
  ...
  return `[out:json][timeout:20];(
way["highway"~"^(footway|path|pedestrian|living_street|residential|secondary|tertiary|primary|service|cycleway)$"](${bb});
);out geom;`
}

function getRoadKind(highway: string): RoadKind {
  if (highway === 'footway' || highway === 'path') return 'footway'
  if (highway === 'pedestrian' || highway === 'living_street') return 'pedestrian'
  if (highway === 'residential' || highway === 'service' || highway === 'cycleway') return 'residential'
  if (highway === 'secondary' || highway === 'tertiary') return 'secondary'
  if (highway === 'primary') return 'primary'
  return 'other'
}

function getRoadWeight(kind: RoadKind): number {
  if (kind === 'pedestrian') return 1.0
  if (kind === 'footway') return 0.9
  if (kind === 'primary') return 0.8
  if (kind === 'secondary') return 0.7
  if (kind === 'residential') return 0.6
  return 0.5
}
...
export function parseRoadResponse(data: OverpassResponse): RoadSegment[] {
  ...
  // cap at 200 segments, prioritise pedestrian/footway
  return segments
